...
from .serializers import (EventSerializer,Reviewserializer,confirmAttendanceSerializer)
...
class confirmAttendanceAPIView(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        attendance_queryset = Attendees.objects.filter(event = self.request.data['event'],user = self.request.user)
...
class ReviewserializerAPIView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        Event_instance = generics.get_object_r_404(Event, pk= Event_pk)
